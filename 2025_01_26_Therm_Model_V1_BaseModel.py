...
BFB = np.array([
    ...
    0.84,   1.14, 0.20, 3*0.50,    # Arms
    0.10,   0.24, 0.04, 3*2.00,    # Hands
    2.69,   3.43, 0.52, 3*2.85,    # Legs
    0.16,   0.02, 0.05, 3*3.00,    # Feet
    0.00                         # Central blood
], dtype=float)
...
T_set = np.array([
    310.25, 308.69, 308.58, 307.65,  # Head
...
for stepIndex in range(numTimeSteps):
    ...
    if active_model.lower() == "stolwijk":
        ...
        SH = (10.0 * (np.tanh(0.48*dTskm + 3.62) - 1.0) * dTskm -27.9 * dThy - 109.8* dTskm_dt_cool )
        ...
        SW = max((320.0 * (ERR[0]-0.5)) + (29.0 * (WRMS - CLDS)),0)#%SW signal [W] Stolwijk -1971
    ...
    for i in range(6):
        ...
        BF[base + 1] = BFB[base + 1] + (Q[base + 1] - Q_bm[base + 1]) / 1.163
